...
print("+-------------------+")
print("|{: <25}|{: <10}|{: <10}|".format("Data Set", "Naive", "Progressive"))
for name,t_naiv,t_prog in zip(dataName,time_naiv,time_prog):
    result = {"name": name, "naive": t_naiv, "progressive": t_prog}
    # Affichage avec délimitations graphiques
    print("+-------------------+")
    print("|{: <25}|{: <10.3f}|{: <10.3f}|".format(result["name"],result["naive"], result["progressive"]))
    print("+-------------------+")
    # print('+-----------+---------------+-----------------+')
